...
class DinoFeaturizer(nn.Module):
    ...
    def __init__(self, dim, cfg):
        ...
        self.feat_type = self.cfg.dino_feat_type #feat
        arch = self.cfg.model_type #vit_base
        self.model =  vit.__dict__["vit_base"](img_size=(518,518),block_chunks=0,ffn_layer="mlp",init_values=1.0,patch_size=14)
        # self.affinity_head=vit.AffinityHead( in_channels=(768,768),
        #                       channels=768, 
        #                       num_classes=27,
        #                       norm_cfg=None,
        ...
        # self.affinity_head.eval().cuda()
        # proxy_head_path="/workspaces/stego/checkpoints/cityscapes/cityscapes_cityscapes_date_Sep26_10-40-57/epoch=3-step=2400.ckpt"
        # state_dict=torch.load(proxy_head_path, map_location="cpu")
        
        # state_dict = state_dict["state_dict"]
        # state_dict = {k.replace("net.affinity_head.", ""): v for k, v in state_dict.items()}
        # self.affinity_head.load_state_dict(state_dict, strict=False)
        if arch == "vit_small":
            self.n_feats = 384
        else:
            self.n_feats = 768
        #2d conv to reduce to 100
    ...
    def forward(self, img, n=1, return_class_feat=False):
        ...
        with torch.no_grad():
            ...
            #downsample image for proxz head
            #img_aff= F.interpolate(img, (224,224), mode="bilinear",
             #                     align_corners=True)
            # get selected layer activations dinov1
            #feat, attn, qkv = self.model.get_intermediate_feat(img, n=n)
            #feat, attn, qkv = feat[0], attn[0], qkv[0]
            #pass image through dinov2
            feat=self.model(img)
            feat=feat["x_prenorm"]
            #pass downsampled image to dinov2
            #feat_aff=self.model(img_aff)
            #feat_aff=feat_aff["outs"]
        ...
        #pass downsampled output to proxz head
        #image_feat_aff=self.affinity_head(feat_aff)
...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def forward(self,
        ...
        coord_shape = [orig_feats.shape[0], self.cfg.feature_samples, self.cfg.feature_samples, 2] #orig_feats.shape(32,768,64,64) feature_samples=11
        ...
        # orig_feats - [B, 768, 16, 16]
        # orig_codes - [B, 100, 64, 64]
