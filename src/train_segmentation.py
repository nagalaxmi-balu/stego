...
#@hydra.main(config_path="/workspaces/stego/src/configs", config_name="train_config.yml")
def my_app(cfg) -> None:
    ...
    trainer.fit(model, train_loader, val_loader,ckpt_path="/workspaces/stego/checkpoints/cityscapes/cityscapes_cityscapes_date_Oct11_09-37-54/epoch=6-step=4422.ckpt")
def load_hydra_cfg(cfg_file) -> DictConfig:

    """Loads a Hydra configuration from the given path of .yaml file.
...
if __name__ == "__main__":
    prep_args()
    cfg_file = "/workspaces/stego/src/configs/train_config.yml"
    asset_info = load_hydra_cfg(cfg_file)
    my_app(asset_info)
