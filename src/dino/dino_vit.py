...
from dino.layers import Mlp, PatchEmbed, SwiGLUFFNFused, MemEffAttention, NestedTensorBlock as Block
import torch.nn.functional as F
#from affinity_view import affinity_view
...
class AffinityHead(nn.Module):
    ...
    def __init__(self, in_channels, channels, num_classes, region_res=(4, 4), norm_cfg=None, act_cfg=dict(type='ReLU'),
        ...
        self.affinity_head = nn.Sequential(
            DepthwiseSeparableConvModule(
                in_channels[0], channels, kernel_size=3, padding=1, act_cfg=act_cfg, norm_cfg=None),
            ConvModule(
    ...
    def init_weights(self):
        super(AffinityHead, self).init_weights()
    ...
    def forward_cls(self, x):
        ...
        # get token logits
        token_logits = (x.permute(0, 2, 3, 1).reshape(B, H * W, -1))  # (B, H * W, C)
        #reducing C to 19 (actual regionproxy code)
        #token_logits = self.mlp(x.permute(0, 2, 3, 1).reshape(B, H * W, -1))  # (B, H * W, C)
        return token_logits

    def forward(self, inputs):
        x_mid, x = inputs  # (B, C, H, W)
        
        x_mid=x_mid.permute(0,3,1,2)
        x=x.permute(0,3,1,2)
        B, _, H, W = x.shape
        ...
        t_out=seg_logits.softmax(dim=1)
        t_out=seg_logits.argmax(dim=1)
        t_out=t_out.squeeze()
        #affinity_view(region,tk,t_out)
...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        #self.aff=AffinityHead(in_channels=(384,384),channels=384, num_classes=19,norm_cfg=None,act_cfg=dict(type='ReLU'))
    ...
    def forward_features_list(self, x_list, masks_list):
        ...
        for x, masks in zip(all_x, masks_list):
            ...
            if -1 in self.out_indices:
                out = x[:, 1:]
                B, N, C = out.shape
                out =out.reshape(B, int(math.sqrt(N)), int(math.sqrt(N)), C)
                outs.append(out)
